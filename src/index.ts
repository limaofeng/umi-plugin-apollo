...
const joinApolloTemplatePath = () => (path: string) => join(__dirname, '../templates', path);
const joinSrcPath = (api: IApi) => (path: string) => join(api.paths.absSrcPath!, path);
...
class Bag {
  ...
  private queryOptionsFile() {
    const { optionsFilename, generateOptionsFile } = getOptionsFile(this as any, this.api);
