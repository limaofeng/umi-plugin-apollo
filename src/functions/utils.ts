...
export const getName = (path: string) => _.lowerFirst(_.startCase(path).replace(/\s/g, ''));
export const getPageTypeName = (name: string) => `${capitalizeFirstLetter(name)}Page`;
...
export const joinApolloPath = (api: IApi, path: string) => join(apolloPath(api), path);
...
export const getOptionsFileInternal = (
  ...
  generateOptionsFile = (): void => {
    const defaultOptionsTemplatePath = joinApolloTemplatePath('default-options.ts');
...
export const getOptionsFile = (bag: IBag, api: IApi) => {
  ...
  const { optionsFilename, generateOptionsFile } = getOptionsFileInternal(bag, api);
  return {
    optionsFilename,
    generateOptionsFile,
  };
