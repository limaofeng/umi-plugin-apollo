...
const getImportSchema = (path: string) =>
  path ? `import typeDefs from '${path}';` : `import typeDefs from './sample.schema.graphql';`;
...
export default (api: IApi, bag: IBag) =>
  api.onGenerateFiles(() => {
    ...
    const linkTemplatePath = bag.joinApolloTemplatePath(options.mock ? 'mock-schema-link.ts' : 'http-link.ts');
